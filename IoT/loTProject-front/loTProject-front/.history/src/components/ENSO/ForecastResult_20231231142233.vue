...
export default {
  ...
  data() {
    return {
      ...
      selectedDate: '2023-01-01', // 初始值为字符串形式
      selectedData: [],
    };
  },
  ...
  methods: {
    updateSelectedData() {
      // 在 @change 事件中，将日期转换为字符串形式
      this.selectedData = this.simulatedData[this.selectedDate] || [];
    },
    ...
    drawLine(id) {
      ...
      charts.setOption({
        ...
        xAxis: {
          type: 'category',
          data: Array.from({ length: this.selectedData.length }, (_, i) => i + 1 + '号'),
        },
        yAxis: {
          type: 'value',
          name: '值',
